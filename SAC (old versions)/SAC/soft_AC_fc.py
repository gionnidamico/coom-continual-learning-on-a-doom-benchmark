...
class SoftQNetwork(nn.Module):
    ...
    def forward(self, state, actions):       # new_action????
        #input is a concatenation of state and actions vectors
        print(state.shape)
        print(actions.shape)
...
class PolicyNetwork(nn.Module):
    ...
    def evaluate(self, state, epsilon=1e-6):
        ...
        # Evaluate the policy and return the action and log probability
        logits, _ = self.forward(state)
        pi_s = Categorical(logits=logits)  # Discrete distribution

        action = pi_s.sample()  # Sample an action
        log_prob = pi_s.log_prob(action).unsqueeze(-1)  # Log probability of the sampled action
    ...
    def get_action(self, state, deterministic=True):
       ...
       # Select an action based on the policy
        logits, _ = self.forward(state)
        if deterministic:
            action = torch.argmax(logits, dim=-1)  # Select the action with the highest logit value
        else:
            pi_s = Categorical(logits=logits)
            action = pi_s.sample()
...
def soft_q_update(batch_size, replay_buffer, gamma=0.99, soft_tau=1e-2):
    ...
    predicted_q_value1 = soft_q_net1(state, current_actions)     # action not used ?
    predicted_q_value2 = soft_q_net2(state, current_actions)
...
if __name__ == "__main__":
    ...
    epsilon_max = 0.9
    epsilon_min = 0.1
    epsilon_decay = 0.005   # should go to min in about 500 steps
    epsilon = epsilon_max
    ...
    if SEQUENCE is None:    # train on single scenario
        ...
        for episode in range(episodes):
            ...
            for step in range(max_steps):
                ...
                # The effective training starts when the replay buffer has at least "batch_size" elements
                if len(replay_buffer) <= batch_size:
                    if np.random.rand() < epsilon:      # epsilon decaying strategy
                        # explore
                    ...
                    action = policy_net.get_action(torch.FloatTensor(state_flattened).unsqueeze(0).to(device), deterministic=exploration_mode) #to(device) ?
