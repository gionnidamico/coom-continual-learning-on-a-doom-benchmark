...
class DoomEnv(BaseEnv):
    ...
    def __init__(self,
        ...
        if render or test_only:  # Use a higher resolution for watching gameplay
            self.game.set_screen_resolution(ScreenResolution.RES_1920X1080)     ### FIXED RES TO WORK ON MY PC
            self.game.add_game_args("+fullscreen 1")                            ### ADDED FULLSCREEN IN RENDER MODE
            self.frame_skip = 1
        elif resolution:  # Use a particular predefined resolution
            self.game.set_screen_resolution(get_screen_resolution(resolution))

        ### THIS EFFECTIVELY STARTS THE VIZDOOM/COOM SCENARIO 
        self.game.init()
