...
if MODEL == 'fc':
    from sac_test_no_RB_no_batch import ValueNetwork, QNetwork, PolicyNetwork
...
def q_loss(state, action, reward, next_state, done, q_net, target_value_net):
    ...
    q_value = q_values.gather(0, action)   # get the Nth q_value that corresponds to the action taken (second dimension of action tensor)
    #print(f"q_values shape: {q_value.shape}, {target_q.shape}")
    return F.mse_loss(q_value, target_q.view(-1))   #target is flattened
