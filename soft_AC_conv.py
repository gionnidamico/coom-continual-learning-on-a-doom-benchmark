...
if __name__ == "__main__":
    ...
    value_net = ValueNetwork(num_frames=4, num_actions=12, num_channels=3, hidden_size=hidden_dim).to(device)
    target_value_net = ValueNetwork(num_frames=4, num_actions=12, num_channels=3, hidden_size=hidden_dim).to(device)

    soft_q_net1 = SoftQNetwork(num_frames=4, num_actions=12, num_channels=3, hidden_size=hidden_dim).to(device)
    soft_q_net2 = SoftQNetwork(num_frames=4, num_actions=12, num_channels=3, hidden_size=hidden_dim).to(device)

    policy_net = PolicyNetwork(num_frames=4, num_actions=12, num_channels=3, hidden_size=hidden_dim, device=device).to(device) # vedi spostamento su device
